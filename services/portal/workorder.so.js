...
WorkorderService.prototype.expectDetailsToBe = function(workorder) {
  ...
  .then((details) => {
    ...
    return Promise.all([
      ...
      .then((workflow) => utils.expect.resultIncludes(workflow, workorder.workflow))
    ]);
  });
};
  ...
  steps.forEach((step, index) => {
    ...
    case 'Vehicle Inspection Step':
      element(by.css(`workorder-result:nth-of-type(${index + 1}) md-list-item:nth-of-type(2) h3`))
        .getText()
        .then(result => expect(result).toEqual(step.tires));
      element(by.css(`workorder-result:nth-of-type(${index + 1}) md-list-item:nth-of-type(3) h3`))
        .getText()
        .then(result => expect(result).toEqual(step.lights));
      break;
    case 'Accident Report Form':
      element(by.css(`workorder-result:nth-of-type(${index + 1}) md-list-item:nth-of-type(1) h3`))
        .getText()
        .then(result => expect(result).toEqual(step.number));
      element(by.css(`workorder-result:nth-of-type(${index + 1}) md-list-item:nth-of-type(2) h3`))
        .getText()
        .then(result => expect(result).toEqual(step.owner));
      element(by.css(`workorder-result:nth-of-type(${index + 1}) md-list-item:nth-of-type(3) h3`))
        .getText()
        .then(result => expect(result).toEqual(step.phone));
      break;
    }
  });
};
